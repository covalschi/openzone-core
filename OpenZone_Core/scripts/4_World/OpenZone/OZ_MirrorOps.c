...
class OZ_MirrorOps
{
    ...
    static string Set(string arg, string op, PlayerIdentity sender, out bool ok, out string error)
    {
        ...
        // МІСТ БЕЗ БОТА -- ВМИКАТИ ДЗЕРКАЛО НЕМА КУДИ (ТЗ-2 R2.6).
        //
        // Міст живий, база працює, чат і новини їдуть -- а гільдії в нього
        // немає зовсім. Без цієї відмови адмін натиснув би кнопку, дістав
        // би Ok від заливки, яка нічого не залила, і пішов би шукати треди,
        // яких ніхто не створить. Причина інша, ніж «міст лежить», і слова
        // теж мусять бути інші.
        if (OZ_BridgeClient.DiscordOff())
        {
            OZ_Log.Warn("mirror: " + kind + " not switched on - the bridge has no Discord bot configured");
            error = "STR_OZ_ERR_NO_DISCORD";
            return "";
        }
    ...
    static bool Write(string kind, bool on)
    {
        ...
        // руками. Перемикач дзеркала, тобто одна булева, затирав цю копію
        // ЩОРАЗУ, і після двох натискань відновити з неї було вже нічого.
