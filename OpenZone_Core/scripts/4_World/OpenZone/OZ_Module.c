...
class OZ_Module : CF_ModuleWorld
{
    ...
    // брудне одним синхронним циклом, а брудними після ролового ресинку
    // ролей стають майже всі онлайн одразу -- вісімдесят файлів в одному
    // кадрі на сервері з одним ядром. П'ять секунд і вісім файлів за тік
    // дають ту саму стелю втрати (черга розходиться швидше, ніж набігає)
    // без видимого хитча.
    ...
    void FlushTick()
    {
        OZ_PlayerStore.FlushSome(FLUSH_PER_TICK);
    }
}
