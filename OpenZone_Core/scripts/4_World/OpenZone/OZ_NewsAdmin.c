...
class OZ_NewsAdminAsk
{
    ...
    // СТЕЛЯ НА ТІЛО ЖИЛА ТУТ, А ТЕПЕР -- В OZ_Const.NEWS_BODY_MAX.
    //
    // Тут стояла своя константа з міркування «власна стеля ядра, а не позика
    // в чужому символі». Ціна виявилась рівно та, від якої це мало вберегти:
    // консоль VPP міряла цим числом, сторінка КПК -- своїм Tuning, і тіло на
    // тисячу байтів одна поверхня приймала, а друга відхиляла (розбіжність
    // 96). Число одне, живе в 3_Game ядра, і клямп Tuning у КПК спирається
    // на нього ж -- розійтися їм більше нема як.
}
...
class OZ_NewsAdminAnswer
{
    ...
    // СКІЛЬКИ БАЙТІВ МОЖНА -- ЧИСЛОМ МОСТА, а не нашим (розбіжність 96).
    // Їде з відмовою body_too_long; на решті нуль. Стеля тут і стеля там --
    // одна тисяча, але поверхня, яка каже своє число, бреше рівно тоді, коли
    // числа розійшлися, тобто саме тоді, коли це важливо.
    int Max = 0;
...
class OZ_NewsSection : OZ_AdminSection
{
    ...
    private string Ask(string route, PlayerIdentity sender, string op, string json, out string error)
    {
        ...
        if (json != "")
        {
            ...
            // Тіло понад стелю -- відмова, і мовчки постити половину речення
            // не можна: кажемо, скільки можна.
            //
            // СТРОГО БІЛЬШЕ, а не «більше або дорівнює»: тіло рівно на
            // тисячу байтів приймала сторінка КПК і відхиляла консоль VPP,
            // тобто дві поверхні одного роду розходились рівно на один байт
            // (розбіжність 96). Порівняння тут те саме, що в КПК і в мосту.
            if (a.Body.Length() > OZ_Const.NEWS_BODY_MAX)
            {
                OZ_Log.Warn("news: body of " + a.Body.Length().ToString() + " b from " + a.Uid + " is over the ceiling, rejected");
