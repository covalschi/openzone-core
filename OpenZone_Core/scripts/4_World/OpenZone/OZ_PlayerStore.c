...
class OZ_PlayerData : OZ_ConfigBase
{
    ...
    //
    // СПАДКОВИХ TransponderMode І TransponderTo ТУТ БІЛЬШЕ НЕМАЄ
    // (2026-09-06). Вони були інертними даними: жоден мод серії їх не читав
    // і не писав, а переносила їх у набір гілка Upgrade(), яку зворотна
    // сумісність, знята власником 2026-09-01, більше нічим не виправдовує.
    // Версія файла НЕ піднімається: JsonFileLoader мовчки пропускає ключ, під
    // який у класі немає поля, тож живий файл зі старими ключами читається
    // далі, а перший же запис їх звідти прибирає.
    ...
    // транспондер.
    //
    // СПАДКОВОГО PresenceHidden ТУТ БІЛЬШЕ НЕМАЄ (2026-09-06) -- з тієї самої
    // причини й на тих самих умовах, що й поля транспондера вище.
    ...
    // РЕЄСТРУ ГРУПОВИХ РОЗМОВ (Chats) ТУТ БІЛЬШЕ НЕМАЄ (2026-09-06).
    //
    // Список id груп персистився з кожним гравцем, і за весь час існування
    // серії його не прочитав ніхто: КПК веде групи власним сховищем, а
    // єдиним дотиком у всіх шести репозиторіях був скид на вайпі у моді
    // фракцій, який мод фракцій уже прибрав.
    //
    // --- належність: ДВІ ОСІ ---
    //
...
//
// ГІЛОК ПЕРЕНЕСЕННЯ СПАДКОВИХ ПОЛІВ ТУТ БІЛЬШЕ НЕМАЄ (2026-09-06): разом із
// самими полями (PresenceHidden, TransponderMode) вони пішли з OZ_PlayerData
// -- див. тамтешні надгробки. Лишилось те, що стосується ЖИВОГО набору й
// мусить статись на кожному завантаженні, а не один раз.
//
