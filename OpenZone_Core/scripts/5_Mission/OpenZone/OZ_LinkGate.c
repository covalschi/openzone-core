...
class OZ_LinkGate
{
    ...
    // Місія скінчилась -- забуваємо ВСЕ, що сказав той сервер.
    //
    // s_Required і s_Done -- статики, а статики переживають місію. Гравець,
    // якого зажадав прив'язки сервер А, від'єднувався й заходив на сервер Б,
    // де OpenZone або немає, або він мовчить -- і на першому ж кадрі йому
    // відкривались НАШІ ворота: на весь екран, без кнопки закриття, з
    // прихованим HUD і заблокованим вводом. OZ_Sync звідти не приходить
    // ніколи, тож зняти їх не могло вже ніщо, крім вбивства гри.
    //
    // Клієнтський мод, що захоплює екран на чужому сервері, -- це те, за що
    // мод вносять у чорні списки. Сусідній мод уже наступив на цей клас
    // помилки з HUD-смужкою (OZ_PdaMissionGameplay.c:50).
    static void Forget()
    {
        s_Required = false;
        s_Done     = false;
