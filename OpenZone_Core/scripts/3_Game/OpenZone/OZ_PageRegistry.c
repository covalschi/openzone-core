...
class OZ_PageHandler
{
    ...
    // ЧИ ВЗАГАЛІ ПОКАЗУВАТИ ЦЮ ВКЛАДКУ ЦЬОМУ ГРАВЦЕВІ.
    //
    // За замовчуванням -- так: більшість сторінок є в кожного, у кого є
    // пристрій, і змушувати кожного автора писати «true» означало б збирати
    // помилки там, де правило одне.
    //
    // Питає СЕРВЕР, на кожен статус, і питає ВЛАСНИКА сторінки -- бо тільки
    // він знає своє правило. КПК не має дізнаватись, що таке «угруповання»,
    // щоб вирішити, чи малювати літеру F у стрічці вкладок: він питає того,
    // хто цю літеру приніс.
    //
    // uid -- ВЛАСНИК СЕСІЇ пристрою, а не той, хто його тримає: пристрій
    // говорить за господаря, і набір вкладок мусить бути його.
    bool VisibleFor(string uid)
    {
        return true;
    }
}
...
class OZ_PageRegistry
{
    ...
    // Зареєстрована І видима цьому гравцеві. Незареєстрована -- завжди ні:
    ...
    static bool VisibleFor(string pageId, string uid)
    {
        ...
        return e.Handler.VisibleFor(uid);
    }
